...
function loadCapacitor(
  ...
  const source = readFileSync(
    join(process.cwd(), 'src/template-electron/src/system/js/electron-init.js'),
    'utf-8',
  );
  const context = {
    URL,
...
describe('electron-init convertFileSrc', () => {
  it('maps known file:// roots to the configured app protocol URL', () => {
    ...
    expect(
      (capacitor.convertFileSrc as (value: string) => string)(
        'file:///Users/example/AppData/images/a%20b.png',
      ),
  ...
  it('keeps the previous no-op behaviour when file src mapping is disabled', () => {
    ...
    expect(
      (capacitor.convertFileSrc as (value: string) => string)(
        'file:///Users/example/AppData/images/a.png',
      ),
...
describe('electron-init nativePromise', () => {
  it('rejects structured plugin failures as Error objects with metadata', async () => {
    ...
    await expect(
      (
        capacitor.nativePromise as (
          plugin: string,
          method: string,
          opts: unknown,
        ) => Promise<unknown>
      )('MyPlugin', 'doThing', null),
