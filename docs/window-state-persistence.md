...
- **macOS:** `~/Library/Application Support/{desktopName}`
- **Windows:** `%APPDATA%\{desktopName}`
- **Linux:** `~/.config/{desktopName}`

`{desktopName}` comes from `electron/package.json` and defaults to the safe package name generated by `cap-electron add` or `cap-electron sync --all`.
